...
char buffer[] = "00:00";
InverterLayer *inv_uhr, *inv_screen;
TextLayer *bt_layer, *batt_layer, *emoji_layer;
static Layer *bti_layer;
...
static void layer_update_callback(Layer *me, GContext* ctx) {
  ...
  GRect bounds = bt_image->bounds;
  graphics_draw_bitmap_in_rect(ctx, bt_image, (GRect) { .origin = { 5, 2 }, .size = bounds.size });
}
...
static void bt_handler(bool connected)
{
  if(connected == true)
  {
    ...
    layer_set_hidden(bti_layer, false);
  }
  else
  {
    //text_layer_set_text(bt_layer, "");
    layer_set_hidden(bti_layer, true);
  }
}
...
void window_load(Window *window) {
  ...
  //Bilder
    ...
    bti_layer = layer_create(bounds);
    layer_set_hidden(bti_layer, true);
    layer_set_update_proc(bti_layer, layer_update_callback);
    layer_add_child(window_layer, bti_layer);
    bt_image = gbitmap_create_with_resource(RESOURCE_ID_BT_ICON);
  ...
  //Batterie
    batt_layer = text_layer_create(GRect(100,-2,39,20));
  ...
  //Emoji-Layer
    emoji_layer = text_layer_create(GRect(5,128,134,18));
  ...
  //Invertierung
    //Statusbar
      //inv_statusbar = inverter_layer_create(GRect(0,0,144,16));
      //layer_add_child(window_get_root_layer(window), inverter_layer_get_layer(inv_statusbar));
    //Uhrzeit in der Mitte
      inv_uhr = inverter_layer_create(GRect(0, 50, 144, 62));
      layer_add_child(window_get_root_layer(window), (Layer*) inv_uhr);
    //Gesamter Screen
      inv_screen = inverter_layer_create(GRect(0,0,144,168));
      layer_add_child(window_get_root_layer(window), (Layer*) inv_screen);
}

void window_unload(Window *window) {
  //We will safely destroy the Window's elements here!
  text_layer_destroy(text_layer);
  inverter_layer_destroy(inv_screen);
  inverter_layer_destroy(inv_uhr);
  text_layer_destroy(bt_layer);
  text_layer_destroy(batt_layer);
  text_layer_destroy(emoji_layer);
  gbitmap_destroy(bt_image);
  layer_destroy(bti_layer);
}
